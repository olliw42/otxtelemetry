...
----------------------------------------------------------------------
local versionStr = "0.31.02 2021-11-15"


-- libraries: tobject.lua, tvehicle.lua, tplay.lua
...
local function drawHudFrameAt(x,y,h)
...
local function drawHudCompassRibbonAt(x,y)
...
local function drawHudGroundSpeedAt(x,y)
...
local function drawHudAltitudeAt(x,y)
...
local function drawHudVerticalSpeedAt(x,y)
    ...
    lcd.drawNumber(x, y-5, verticalSpeed*10, CUSTOM_COLOR+MIDSIZE+CENTER+PREC1)
end

local function drawHudAt(x,y,h)
    drawHudFrameAt(x, y, h)
    drawHudCompassRibbonAt(x, y)
    drawHudGroundSpeedAt(x - 120, y+58)
    drawHudAltitudeAt(x + 120, y+58)
    drawHudVerticalSpeedAt(x, y+h-18)
end    
...
local function drawGpsStatusAt(gpsId, x,y, dy)
    ...
    if gpsId > 0 then
        lcd.drawText(x, y+8, getGpsFixStr(), CUSTOM_COLOR+txtsize1+LEFT) --MIDSIZE+LEFT)
    else
        local fixstr = "No FIX"
        if gpsfix >= mavlink.GPS_FIX_TYPE_3D_FIX then
...
local function gpsLatLonToDms(dec,lat)
...
local function drawGpsCoordsAt(sourceId, x,y)
    ...
    lcd.setColor(CUSTOM_COLOR, p.WHITE)
    lcd.drawText(x, y, gpsLatLonToDms(lat), CUSTOM_COLOR)
    lcd.drawText(x, y+16, gpsLatLonToDms(lon,lat), CUSTOM_COLOR)
end
...
local function drawBatteryVoltageAt(x,y)
...
local function drawBatteryCurrentAt(x,y)
...
local function drawBatteryRemainingAt(x,y)
...
local function drawBatteryChargeAt(x,y)
...
local apdraw = loadLib("tautopilot.lua")
...
local function drawAutopilotPage()
    --drawHud()
    drawHudAt(draw.xmid, 22, 146)
    
    -- drawGpsStatus()
    if not mavsdk.isGps2Available() then 
        drawGpsStatusAt(1, 2,30, 5); 
    else
    ...
    if mavsdk.isTxGpsAvailable() then 
        ...
        drawGpsStatusAt(0, 2,y, 0) --drawTxGpsStatusAt(2, y)
    end
    ...
    drawGpsCoordsAt(1, 2,165)
    
    -- draw Battery Status
    drawBatteryVoltageAt(draw.xsize, 30)
    drawBatteryCurrentAt(draw.xsize, 65)
    drawBatteryRemainingAt(draw.xsize, 100)
    drawBatteryChargeAt(draw.xsize, 135)
