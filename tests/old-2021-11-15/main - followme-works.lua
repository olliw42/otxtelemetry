...
----------------------------------------------------------------------
local versionStr = "0.30.15 2021-08-22"
...
local function gimbalSetQShotMode(mode, sound_flag)
    gimbal.qshot_mode = mode
    mavsdk.qshotSendCmdConfigure(gimbal.qshot_mode, 0)
    if not sound_flag then return end
    if mode == mavsdk.QSHOT_MODE_DEFAULT then
        play:QShotDefault()
    elseif mode ==  mavsdk.QSHOT_MODE_NEUTRAL then
        play:QShotNeutral()
    elseif mode == mavsdk.QSHOT_MODE_RC_CONTROL then
        play:QShotRcControl()
    elseif mode == mavsdk.QSHOT_MODE_POI then
        play:QShotPOI()
    elseif mode == mavsdk.QSHOT_MODE_SYSID then
        --play:QShotSysid()
        play:QShotTargetMe()
    elseif mode == mavsdk.QSHOT_MODE_CABLECAM then
        play:QShotCableCam()
    end
end  
...
local function gimbal_menu_click(menu, idx)
    gimbalSetQShotMode(menu.qshot_mode[idx], true)
end
...
--    qshot_mode = { mavsdk.QSHOT_MODE_DEFAULT, mavsdk.QSHOT_MODE_NEUTRAL, mavsdk.QSHOT_MODE_RC_CONTROL,
--                   mavsdk.QSHOT_MODE_POI, mavsdk.QSHOT_MODE_CABLECAM },
    option = { "Default", "Neutral", "RC Control", "Target Me" },
    qshot_mode = 
             { mavsdk.QSHOT_MODE_DEFAULT, mavsdk.QSHOT_MODE_NEUTRAL, mavsdk.QSHOT_MODE_RC_CONTROL,
               mavsdk.QSHOT_MODE_SYSID },
}
...
local function gimbalSetPitchYawDeg(pitch, yaw)
    if not config_g.gimbalUseGimbalManager or not mavsdk.gimbalIsProtocolV2() then
        ...
        elseif gimbal.qshot_mode == mavsdk.QSHOT_MODE_SYSID then
            mavsdk.gimbalClientSetFlags(mavsdk.GMFLAGS_AUTOPILOT_ACTIVE)
        end
...
local function gimbalDoAlways()
    ...
    if gimbal.qshot_init_cnt > 0 then
        ...
        if gimbal.qshot_init_cnt >= 3 then
            gimbalSetQShotMode(gimbal_menu.qshot_mode[gimbal_menu.idx], false)
        else  
            gimbalSendRoiSysId()
        end    
    end
...
local function actionDoAlways()
    ...
    if not action.initialized then
        ...
        tmenu_init(action_follow_menu)
        tbuttonlong_init(action_follow_button)
    end
...
--isthrok = true
    ...
    tobject_enable(action_follow_menu, follow.params_initialized)
    tobject_enable(action_follow_button, follow.params_initialized)
    ...
    tmenu_handle(action_follow_menu)
    tbuttonlong_handle(action_follow_button)
    ...
    tbuttonlong_draw(action_arm_button)
    tbuttonlong_draw(action_follow_button)
