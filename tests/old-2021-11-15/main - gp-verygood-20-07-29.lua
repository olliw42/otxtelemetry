...
----------------------------------------------------------------------
local versionStr = "0.30.10 2021-07-29"
...
local camera_status_last = nil

local camera_initialized = false
local camera_params_initialized = false
local camera_params_last = 0
local camera_params_tries = 0
local camera_isshooting = nil
...
local camera_param_registered = false

local function camera_register_params()
    if camera_param_registered then return end
    local cam_sysid, cam_compid = mavlink.getCameraIds()
...
local function cameraDoAlways(bkgrd)
    if not mavsdk.cameraIsReceiving() then
      camera_status_last = nil
      camera_params_initialized = false
      camera_params_tries = 0
      for i = 1, #camera_menu_list do camera_menu_list[i].idx = -10 end
      camera_param_registered = false
      mavlink.clearParamRegister()
    end
    if not mavsdk.cameraIsInitialized() then return end

    if not camera_initialized then
    ...
    if camera_status_last == nil then
        camera_status_last = status
        if status.mode == mavlink.CAMERA_MODE_VIDEO then 
            camera_menu.idx = 1
        end
    ...
    if camera_status_last.mode ~= status.mode then
        if status.mode == mavlink.CAMERA_MODE_VIDEO then 
            playVideoMode() 
            camera_menu.idx = 1
        end
    ...
    if not camera_params_initialized and camera_params_tries < 50 then
        local tnow = getTime()
        local ok = true
        for i = 1, #camera_menu_list do
            local menu = camera_menu_list[i]
            if menu.idx < 0 then
                if menu.idx == -10 or (tnow - camera_params_last) > 100 then
                    menu.idx = -9
                    camera_params_last = tnow
                    camera_params_tries = camera_params_tries + 1
--                    cam_request_param(menu.cmd)
                    mavlink.sendParamRequest(menu.handle)
                end
              ok = false
...
local function doPageCamera()
    ...
    if camera_isshooting == nil or shooting ~= camera_isshooting then
        camera_isshooting = shooting
    ...
    if info.has_video and camera_params_initialized and status.mode == mavlink.CAMERA_MODE_VIDEO then 
        for i = 1, #VID_menu_list do 
            tmenu_handle(VID_menu_list[i]) 
        end    
    end    
    if info.has_photo and camera_params_initialized and status.mode == mavlink.CAMERA_MODE_IMAGE then 
