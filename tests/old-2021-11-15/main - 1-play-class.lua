...
----------------------------------------------------------------------
local versionStr = "0.30.12 2021-08-01"
...
function play:TakeOff() end --fplay("takeoff") end    
function play:MagCalibrationStarted() end
function play:MagCalibrationFinished() end
...
local function checkStatusChanges()
    ...
    if status_g.flightmode == nil or status_g.flightmode ~= mavsdk.getFlightMode() then -- first call or change
        ...
        if mavsdk.isReceiving() then play:FlightModeSound() end
    end
...
local function tbuttonlong_handle(button)
    ...
    if touchEventTap(button.rect) or touchEventPressed(button.rect) then 
        button.pressed = true
        if button.press_start == nil then button.press_start = getTime() end
...
--[[
  ...
  - requires flight mode which allows it: PosHold, Guided, Loiter, if not must_navigate: AltHold, FlowHold, Sport
    ...
    action.istakingoff = true
    action.istakingoff_start = getTime()
    play:TakeOff()
end
...
local function actionDoAlways()
    ...
    local flightmode = mavsdk.getFlightMode()
    if not action.islanding and not flightmode == apCopterFlightMode.Land then 
        action.landing_flightmode_at_start = flightmode
    end    
    if action.islanding then 
        if mavsdk.getFlightMode() == flightmode then
            if not mavsdk.isArmed() then
    ...
    if action.landing_restore_flightmode_cntdown > 0 then 
        if getTime() > action.landing_restore_flightmode_cntdown then
            action.landing_restore_flightmode_cntdown = 0
            mavsdk.setFlightMode(action.landing_flightmode_at_start)
            action.landing_flightmode_at_start = nil
        end
    end    
...
local function doPageAction()
    ...
    local isthrok = (thr >= -50) and (thr <= 50)
    ...
    if action.istakingoff then
        lcd.drawText(draw.xmid+60, 200, "TAKE OFF", CUSTOM_COLOR+DBLSIZE)
        if (getTime() - action.istakingoff_start) > 200 then 
            action.istakingoff = false
        end    
    elseif action_takeoff_button.armed then
        lcd.drawText(draw.xmid+60, 200, "ARM", CUSTOM_COLOR+DBLSIZE)
    end  
...
local function doAlways(bkgrd)
    ...
    if pageCameraEnabled then cameraDoAlways() end
    if pageGimbalEnabled then gimbalDoAlways() end
    if pageActionEnabled then actionDoAlways() end
end
