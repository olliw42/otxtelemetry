...
----------------------------------------------------------------------
local versionStr = "0.30.15 2021-08-07"
...
local function tobject_visible(tobject,flag)
    tobject.visible = flag
end  

local function tobject_enable(tobject,flag)
    tobject.enabled = flag
end  

local function tobject_init(tobject,flag)
    if not tobject.initialized then
        tobject.initialized = true
        tobject.enabled = true
        if flag then tobject.visible = flag else tobject.visible = true
        end
    end
end


local function tbutton_draw(button)
...
local function tbutton_handle(button)
    if not button.visible then return end
    if not button.enabled then return end
    if tobject_has_focus then return end
    if touchEventTap(button.rect) then button.click_func(button) end
end  

local function tbutton_init(button, flag)
    if not button.initialized then
        tobject_init(button, flag)
    end    
end


local function tmenu_popup_rect(menu)
  return {x = menu.px, y = menu.py, w = menu.w, h = menu.ph} 
end
  
local function tmenu_popup_touch(menu)
    if touch == nil then return end
    for i = menu.min, menu.max do
        if touch.y > menu.py + menu.h*(i-1) and touch.y < menu.py + menu.h*i then
            menu.m_idx = i
            return
        end
    end  
...
local function tmenu_popup_draw(menu)
    local size = MIDSIZE
...
local function tmenu_draw(menu)
    ...
    if menu.active then
        ...
        lcd.drawRectangle(r.x, r.y, r.w, r.h, CUSTOM_COLOR+SOLID)
        lcd.drawRectangle(r.x+1, r.y+1, r.w-2, r.h-2, CUSTOM_COLOR+SOLID)
        tmenu_popup_draw(menu)
    end
end

local function tmenu_handle(menu)
    if not menu.visible then return end
    if not menu.enabled then return end
    if not tobject_has_focus and not menu.active and touchEventTap(menu.rect) then
        menu.active = true
        tobject_has_focus = true
        menu.m_idx = menu.idx
    elseif menu.active then
        if touchEventTap(tmenu_popup_rect(menu)) then
            tmenu_popup_touch(menu)
            menu.active = false
...
local function tmenu_init(menu, flag)
    if not menu.initialized then
        tobject_init(menu, flag)
...
local function cameraDoAlways()
    ...
    if not camera.initialized then
        ...
        tmenu_init(camera_menu)
        for i = 1, #camera_menu_list do 
          tmenu_init(camera_menu_list[i])
...
local function doPageCamera()
    ...
    if camera.isshooting == nil or shooting ~= camera.isshooting then
        ...
        tobject_enable(camera_menu, not shooting)
        for i = 1, #camera_menu_list do tobject_enable(camera_menu_list[i], not shooting) end
    end    
    ...
    if info.has_video and camera.params_initialized and status.mode == mavlink.CAMERA_MODE_VIDEO then 
        for i = 1, #VID_menu_list do
            tmenu_draw(VID_menu_list[i])
        end
    end
    if info.has_photo and camera.params_initialized and status.mode == mavlink.CAMERA_MODE_IMAGE then 
        for i = 1, #PHO_menu_list do
            tmenu_draw(PHO_menu_list[i])
        end
    end
...
local function gimbal_menu_click(idx)
    gimbalSetQShotMode(idx, true)
end
...
local function gimbalSetPitchYawDeg(pitch, yaw)
    if not config_g.gimbalUseGimbalManager or not mavsdk.gimbalIsProtocolV2() then
        ...
        if gimbal.qshot_mode == mavsdk.QSHOT_MODE_DEFAULT then
            mavsdk.gimbalClientSetFlags(mavsdk.GMFLAGS_GCS_ACTIVE)
        elseif gimbal.qshot_mode == mavsdk.QSHOT_MODE_NEUTRAL then
            mavsdk.gimbalClientSetNeutral(1)
            mavsdk.gimbalClientSetFlags(0)
        elseif gimbal.qshot_mode == mavsdk.QSHOT_MODE_RC_CONTROL then
            mavsdk.gimbalClientSetFlags(mavsdk.GMFLAGS_RC_ACTIVE)
        elseif gimbal.qshot_mode == mavsdk.QSHOT_MODE_POI then
...
local function tbuttonlong_draw(button)
--    tbutton_draw(button)
    if not button.visible then return end
    local r = button.rect
    lcd.setColor(CUSTOM_COLOR, p.BLUE)
...
local function tbuttonlong_handle(button)
    if not button.visible or not button.enabled or tobject_has_focus then
        button.press_start = nil; 
        return false
    end
    if touchEventTap(button.rect) or touchEventPressed(button.rect) then 
        button.pressed = true
...
local function tbuttonlong_init(button)
    if not button.initialized then
        tbutton_init(button)
        button.pressed = false
        button.clicked = false
...
-- arm button class
local function tarmbutton_draw(button)
    if not button.visible then return end
    local r = button.rect
    lcd.setColor(CUSTOM_COLOR, p.BLUE)
    lcd.drawFilledRectangle(r.x, r.y, r.w, r.h, CUSTOM_COLOR+SOLID)
...
local function ttakeoffbutton_handle(button)
    local res = tbuttonlong_handle(button)
    if res then
...
local follow = {
    ...
    params_tries = 0,
    param_list = {
        { param_id = "FOLL_OFS_TYPE", idx = -10, handle = nil, val = 0, desired_val = 0 },
        { param_id = "FOLL_YAW_BEHAVE", idx = -10, handle = nil, val = 0, desired_val = 0 },
        { param_id = "FOLL_ENABLE", idx = -10, handle = nil, val = 0, desired_val = 1 },
        { param_id = "FOLL_ALT_TYPE", idx = -10, handle = nil, val = 0, desired_val = 1 },
        { param_id = "FOLL_SYSID", idx = -10, handle = nil, val = 0, desired_val = 254 },
        { param_id = "FOLL_OFS_X", idx = -10, handle = nil, val = 0, desired_val = 0 },
        { param_id = "FOLL_OFS_Y", idx = -10, handle = nil, val = 0, desired_val = 0 },
        { param_id = "FOLL_OFS_Z", idx = -10, handle = nil, val = 0, desired_val = 0 },
        --{ param_id = "FOLL_DIST_MAX", idx = -10, handle = nil, val = 0, desired_val = 100 },
        --{ param_id = "FOLL_POS_P", idx = -10, handle = nil, val = 0, desired_val = 0 }
    }
}  

local function action_followNED_button_press()
    local param = follow.param_list[1]
    param.desired_val = 0
    mavlink.sendParamSet(param.handle, param.desired_val)
end

local function action_followNED_button_click()
    local ap_sysid, ap_compid = mavlink.getAutopilotIds()
end

local action_followNED_button = {
    rect = { x = 330, y = 165, w = 140, h = 34 },
    txt = "Follow NED",
    press_func = action_followNED_button_press,
    click_func = action_followNED_button_click,
}

local function action_followREL_button_press()
    local param = follow.param_list[1]
    param.desired_val = 1
    mavlink.sendParamSet(param.handle, param.desired_val)
...
local function action_followREL_button_click()
    local ap_sysid, ap_compid = mavlink.getAutopilotIds()
end

local action_followREL_button = {
    rect = { x = 330, y = 205, w = 140, h = 34 },
    txt = "Follow REL",
    press_func = action_followREL_button_press,
    click_func = action_followREL_button_click,
}
...
local function follow_set_params_to_desired()
    if not follow.param_registered then return end
    local ok = true
    for i = 1, #follow.param_list do
        local param = follow.param_list[i]
        if param.idx < 0 then
            if param.idx == -10 then --decimate
                param.idx = -9
...
local function follow_params_check()
    if not follow.param_registered then return end
    local failed_i = 0
    for i = 1, #follow.menu_list do
        local res = mavlink.getParamValue(follow.menu_list[i].handle)
        if res.updated then
            local val = mavlink.memcpyToInteger(res.param_value)
            follow.menu_list[i].val = val
            if (follow.menu_list[i].desired_val ~= val) then 
                failed_i = i
            end
        end
    end
...
local function actionDoAlways()
    ...
    if not action.initialized then
        ...
        tbuttonlong_init(action_land_button)
        tbuttonlong_init(action_arm_button)
        tbuttonlong_init(action_compass_start_button)
        tbutton_init(action_compass_cancel_button)
        ttakeoffbutton_init(action_takeoff_button)
        tbuttonlong_init(action_followNED_button)
        tbuttonlong_init(action_followREL_button)
    end
    
--[[    
    if mavsdk.isReceiving() and not follow.params_initialized then
        if follow_set_params_to_desired() then
            local failed_i = follow_params_check()
            if failed_i == 0 then 
                follow.params_initialized = true 
            else
                follow.param_list[failed_i].idx = -9
            end
        end   
    end
]]    
...
--[[    
    ...
    tobject_enable(action_land_button, isarmed)
    tobject_enable(action_compass_start_button, not isarmed and not action.magcalibrationstarted)
    tobject_visible(action_compass_cancel_button, action.magiscalibrating)
...
--    tobject_enable(action_takeoff_button, isarmed and istakeoffable and isthrok)
--istakeoffable = true
--isthrok = true
    tobject_enable(action_takeoff_button, istakeoffable and isthrok)
    
    tbuttonlong_handle(action_land_button)
    tbuttonlong_handle(action_arm_button)
    tbuttonlong_handle(action_compass_start_button)
    tbutton_handle(action_compass_cancel_button)
    ttakeoffbutton_handle(action_takeoff_button)
    tbuttonlong_handle(action_followNED_button)
    tbuttonlong_handle(action_followREL_button)
    ...
    tbuttonlong_draw(action_land_button)
    tarmbutton_draw(action_arm_button)
    tbuttonlong_draw(action_followNED_button)
    tbuttonlong_draw(action_followREL_button)
    tbuttonlong_draw(action_compass_start_button)
    tbutton_draw(action_compass_cancel_button)
...
local function widgetRefresh(widget, events)
    ...
    if pages[page.idx] == page.IdAutopilot and #statustext < 3 then
        lcd.setColor(CUSTOM_COLOR, p.GREY)
