...
local function drawPrearm()
    ...
    local arming_check = mavsdk.apGetArmingCheck()
    if arming_check == nil then arming_check = 0 end  
  
    y = 75+25+35;
    lcd.setColor(CUSTOM_COLOR, p.LIGHTGREY)
    local ii = 0;
    if arming_check == nil then 
    elseif bit32.btest(arming_check, apPrearmChecks[0][2]) then -- All
        lcd.drawText(x, y+14*ii, apPrearmChecks[0][1], CUSTOM_COLOR+SMLSIZE)
    else  
        for i=1,#apPrearmChecks do
            if bit32.btest(arming_check, apPrearmChecks[i][2]) then 
                lcd.drawText(x, y+14*ii, apPrearmChecks[i][1], CUSTOM_COLOR+SMLSIZE)
                ii = ii + 1
                if ii == 9 then 
                    x = x+110
                    ii = 0
    ...
    -- we could make it also dependent on camera and gimbal, if we want this!
    if autopilot_ok then
        lcd.setColor(CUSTOM_COLOR, p.GREEN)
        lcd.drawText(draw.xmid, 20-4, "PREARM  OK", CUSTOM_COLOR+DBLSIZE+CENTER)
    else  
        lcd.setColor(CUSTOM_COLOR, p.RED)
        lcd.drawText(draw.xmid, 20-4, "PREARM  FAIL", CUSTOM_COLOR+DBLSIZE+CENTER)
        lcd.drawText(10, 20-4 + 35, string.sub(prearm_last_statustext,9), CUSTOM_COLOR+SMLSIZE)
    end
end
