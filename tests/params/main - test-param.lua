...
local inout_enabled = false
local last_param = 0

local function str2tab(s)
  local t = {}
  --for i=1, 16 do t[i-1] = 0 end
  for i=1, string.len(s) do t[i-1] = string.byte(string.sub(s,i,i)) end
  return t
end
...
local function widgetRefresh(widget, events)
    ...
    if mavsdk.isReceiving() then --and not mavsdk.gimbalIsReceiving() then
        ...
        local tnow = getTime() -- 10ms
        if (tnow - last_param) > 100 then
            last_param = tnow
            lcd.drawText(LCD_W/2, 20, "send", CUSTOM_COLOR+CENTER)
            
            
            --send message
            local param_id_s = "text"
            local param_id_t = {}
            --for i = 1, #param_id_s do param_id_t[i] = param_id_s:sub(i, i) end
            for i=1, string.len(param_id_s) do
                param_id_t[i] = (string.sub(param_id_s,i,i))
            end
            local param_msg = {
              msgid = mavlink.M_PARAM_REQUEST_READ, --20,
              target_sysid = 1,
              target_compid = 1,
            ...
            local res = mavlink.sendMessage(param_msg)
            if res == nil then
              lcd.drawText(LCD_W/2, 40, "N I L", CUSTOM_COLOR+CENTER)
            elseif res == true then
              lcd.drawText(LCD_W/2, 40, "OK", CUSTOM_COLOR+CENTER)
            else
              lcd.drawText(LCD_W/2, 40, "????????????", CUSTOM_COLOR+CENTER)
            end  
        end  
      
        --getMessage
        local param_value = mavlink.getMessage(22)
        if param_value ~= nil then
            ...
            if param_value.updated then
              ...
              local s = tab2str(param_value.param_id)
              lcd.drawText(LCD_W/2, 140, "param_id  "..s, CUSTOM_COLOR+CENTER)
              s = string.format("%s  %d", "param_value", param_value.param_value)
