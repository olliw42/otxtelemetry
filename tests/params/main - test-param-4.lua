...
local function tmenu_init(menu, flag)
    if not menu.initialized then
        ...
        menu.px = draw.xmid - menu.w/2
        menu.py = draw.ymid - menu.ph/2
    end
end
...
local function VIDRES_menu_click(idx)
    ap_set_param("MNT_ZFLAGS", idx)
    ap_request_param("MNT_ZFLAGS")
end

local VIDRES_menu = { 
    rect = { x = 360, y = 20, w = 120, h = 37 },
    min = 1, max = 4, default = 2, 
    option = { "4K", "2.7K", "2.7K 4:3", "1440" },
    click_func = VIDRES_menu_click,
    gp = { 1, 4, 6, 7 },
}
...
local function VIDEV_menu_click(idx)
    ap_request_param("MNT_ZFLAGS")
end

local VIDEV_menu = { 
    rect = { x = 360, y = 140, w = 120, h = 37 },
    min = 1, max = 9, default = 5, 
    option = { "+2.0", "+1.5", "+1.0", "+0.5", "+-0", "-0.5", "-1.0", "-1.5", "-2.0" },
    click_func = VIDEV_menu_click,
    popup_text_size = 0, popup_text_height = 27,
    gp = { 0, 1, 2, 3, 4, 5, 6, 7, 8 },
}

local function VIDSPD_menu_click(idx)
    ap_request_param("MNT_ZFLAGS")
end

local VIDSPD_menu = { 
    rect = { x = 360, y = 180, w = 120, h = 37 },
    min = 1, max = 8, default = 1, 
    option = { "auto", "1/30", "1/60", "1/120", "1/180", "1/240", "1/360", "1/480" },
    click_func = VIDSPD_menu_click,
    popup_text_size = 0, popup_text_height = 28,
    gp = { 0, 5, 8, 13, 15, 18, 20, 22 },
}

local function VIDEIS_menu_click(idx)
    ap_request_param("MNT_ZFLAGS")
end

local VIDEIS_menu = { 
    rect = { x = 360, y = 220, w = 120, h = 37 },
    min = 1, max = 2, default = 1, 
    option = { "EIS off", "EIS on" },
    click_func = VIDEIS_menu_click,
}
...
local function widgetRefresh(widget, events)
    ...
    tmenu_init(VIDRES_menu)
    tmenu_init(VIDFPS_menu)
    tmenu_init(VIDFOV_menu)
    tmenu_init(VIDEV_menu)
    tmenu_init(VIDSPD_menu)
    tmenu_init(VIDEIS_menu)
    
    if mavsdk.isReceiving() then
      tmenu_handle(VIDRES_menu)
      tmenu_handle(VIDFPS_menu)
      tmenu_handle(VIDFOV_menu)
      tmenu_handle(VIDEV_menu)
      tmenu_handle(VIDSPD_menu)
      tmenu_handle(VIDEIS_menu)
    end
    ...
    if mavsdk.isReceiving() then --and not mavsdk.gimbalIsReceiving() then
        ...
        if param_value ~= nil and param_value.updated then
            ...
            param_val = param_value.param_value
        end    
    ...
    tmenu_draw(VIDRES_menu)
    tmenu_draw(VIDFPS_menu)
    tmenu_draw(VIDFOV_menu)
    tmenu_draw(VIDEV_menu)
    tmenu_draw(VIDSPD_menu)
    tmenu_draw(VIDEIS_menu)
