...
----------------------------------------------------------------------
local versionStr = "0.30.8 2021-07-29"
...
local function tobject_init(tobject)
...
--local function tbutton_click(button)
--    return touchEventTap(button.rect)
--end  
...
local function tbutton_init(button)
...
local function tmenu_popup_rect(menu)
  local ph = menu.rect.h * (menu.max - menu.min + 1)
  return {x = draw.xmid - menu.rect.w/2, y = draw.ymid - ph/2, w = menu.rect.w, h = ph} 
end
...
local function tmenu_popup_draw(menu)
    ...
    lcd.drawFilledRectangle(px, py, w, ph, CUSTOM_COLOR+SOLID)
    lcd.setColor(CUSTOM_COLOR, p.GREEN)
    lcd.drawFilledRectangle(px, py + h*(menu.m_idx-1), w, h, CUSTOM_COLOR+SOLID)
    lcd.setColor(CUSTOM_COLOR, p.WHITE)
    lcd.drawRectangle(px, py, w, ph, CUSTOM_COLOR+SOLID)
    lcd.setColor(CUSTOM_COLOR, p.WHITE)
    for i = menu.min, menu.max do
        lcd.drawText(draw.xmid, py + 3 + h*(i-1), menu.option[i], CUSTOM_COLOR+MIDSIZE+CENTER)
    end    
end
...
local function tmenu_init(menu)
...
local function tmenu_handle(menu)
    if not menu.visible then return end
    if not menu.active and touchEventTap(menu.rect) then
        menu.active = true
        menu.m_idx = menu.idx
...
local param_registered = false
local handleDUMMY = nil
local handleDUMMY2 = nil


local VIDRES_value = nil

local function VIDRES_button_click()
  
    mavlink.sendParamSet(handleDUMMY, mavlink.memcpyToNumber(25))
    mavlink.sendParamRequest(handleDUMMY)
    mavlink.sendParamRequest(handleDUMMY)
...
--[[
    mavlink.sendMessage({
        ...
        target_sysid = 1,
        target_compid = 105,
        param_id = str2tab("DUMMY"),
        param_value = mavlink.memcpyToNumber(25),
    ...
    mavlink.sendMessage({
        ...
        target_sysid = 1,
        target_compid = 105,
        param_id = str2tab("DUMMY"), --"DUMMY",
    })
]]
--    mavlink.sendMessage({
--[[
    mavlink.sendParamSet({
        msgid = mavlink.M_PARAM_SET,
        target_sysid = 1,
        target_compid = ap_compid,
        param_id = "MNT_ZFLAGS",
...
local VIDRES_button = { 
    rect = { x = draw.xmid - 45, y = 180, w = 90, h = 37 },
    txt = "SET", --"RES",
    click_func = VIDRES_button_click,
}


local VIDFOV_value = nil

local function VIDFOV_WIDE_button_click()
...
--[[
    ...
    --if res then playTakePhoto() else playModeChangeFailed() end
end

local VIDFOV_WIDE_button = { 
    rect = { x = draw.xmid - 45, y = 220, w = 90, h = 37 },
    txt = "RQST", --"WIDE",
    click_func = VIDFOV_WIDE_button_click,
}
...
local function widgetRefresh(widget, events)
    ...
    --mavlink.enableIn(1)
    --mavlink.enableOut(1)
    if not param_registered then --handleDUMMY == nil then
        param_registered = true
        handleDUMMY = mavlink.registerParam(1, 105, "DUMMY", mavlink.PARAM_TYPE_UINT32)
        handleDUMMY2 = mavlink.registerParam(1, 105, "DUMMY2", mavlink.PARAM_TYPE_UINT32)
    end        
    ...
    tbutton_init(VIDRES_button)
    tbutton_init(VIDFOV_WIDE_button)
    tobject_visible(VIDRES_button, true)
    tobject_visible(VIDFOV_WIDE_button, true)
    
    if mavsdk.isReceiving() then
      tbutton_handle(VIDRES_button)
      tbutton_handle(VIDFOV_WIDE_button)
    end

    lcd.drawText(10, 10, "!"..param_id.."!", CUSTOM_COLOR)
    lcd.drawNumber(10, 30, param_val, CUSTOM_COLOR)
    lcd.drawNumber(10, 50, param_valcvt, CUSTOM_COLOR)

    lcd.drawNumber(10, 100, handleDUMMY, CUSTOM_COLOR)
    lcd.drawNumber(10, 120, handleDUMMY2, CUSTOM_COLOR)

    if mavsdk.isReceiving() then --and not mavsdk.gimbalIsReceiving() then
        --getMessage
--[[        
        local msg = mavlink.getMessage(mavlink.M_PARAM_VALUE) --22
--        local msg = mavlink.getParamValue(mavlink.M_PARAM_VALUE) --22
        if msg ~= nil and msg.updated then
            param_id = tab2str(msg.param_id)
            --param_id = msg.param_id
            param_val = msg.param_value
            param_valcvt = mavlink.memcpyToInteger(msg.param_value)
        end    
        
        if msg ~= nil then
            lcd.drawText(LCD_W/2, 100, "RECEIVED", CUSTOM_COLOR+CENTER)
            if msg.updated then
              lcd.drawText(LCD_W/2, 120, "updated", CUSTOM_COLOR+CENTER)
              lcd.drawText(LCD_W/2, 140, "param_id  "..tab2str(msg.param_id), CUSTOM_COLOR+CENTER)
              --lcd.drawText(LCD_W/2, 140, "param_id  "..msg.param_id, CUSTOM_COLOR+CENTER)
              local s = string.format("%s  %d", "param_value", msg.param_value)
              lcd.drawText(LCD_W/2, 160, s, CUSTOM_COLOR+CENTER)
            else  
              lcd.drawText(LCD_W/2, 120, "old", CUSTOM_COLOR+CENTER)
            end  
        else  
            lcd.drawText(LCD_W/2, 100, "N I L", CUSTOM_COLOR+CENTER)
        end
]]        
        local res = mavlink.getParamValue(handleDUMMY2)
        if not res.updated then res = mavlink.getParamValue(handleDUMMY) end    
        if res.updated then
            param_id = res.param_id
            param_val = res.param_value
            param_valcvt = mavlink.memcpyToInteger(res.param_value)
        end    
        ...
        if res ~= nil then
            ...
            if res.updated then
              lcd.drawText(LCD_W/2, 120, "updated", CUSTOM_COLOR+CENTER)
              lcd.drawText(LCD_W/2, 140, "param_id  "..param_id, CUSTOM_COLOR+CENTER)
              local s = string.format("%s  %d", "param_value", param_val)
              lcd.drawText(LCD_W/2, 160, s, CUSTOM_COLOR+CENTER)
            else  
              lcd.drawText(LCD_W/2, 120, "old", CUSTOM_COLOR+CENTER)
            end  
    ...
    tbutton_draw(VIDRES_button)
    tbutton_draw(VIDFOV_WIDE_button)
