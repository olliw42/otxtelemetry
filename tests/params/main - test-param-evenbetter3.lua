...
local VIDFPS_value = nil

local function VIDFPS_menu_click(idx)
    local ap_sysid, ap_compid = mavlink.getAutopilotIds()
    mavlink.sendMessage({
        msgid = mavlink.M_PARAM_REQUEST_READ,
        target_sysid = ap_sysid,
...
local VIDFPS_menu = { 
    rect = { x = 360, y = 100, w = 90, h = 37 },
    min = 1, max = 2, default = 1, 
    option = { "30 fps", "60 fps" },
    click_func = VIDFPS_menu_click;
}


local function VIDFOV_menu_click(idx)
    local ap_sysid, ap_compid = mavlink.getAutopilotIds()
    mavlink.sendMessage({
        msgid = mavlink.M_PARAM_REQUEST_READ,
        target_sysid = ap_sysid,
...
local VIDFOV_menu = { 
    rect = { x = 360, y = 140, w = 90, h = 37 },
    min = 1, max = 2, default = 1, 
    option = { "wide", "flat" },
    click_func = VIDFOV_menu_click;
}
...
local function widgetRefresh(widget, events)
    ...
    tmenu_init(VIDFPS_menu)
    tmenu_init(VIDFOV_menu)
    tobject_visible(VIDRES_button, true)
    tobject_visible(VIDFOV_WIDE_button, true)
    tobject_visible(VIDFPS_menu, true)
    tobject_visible(VIDFOV_menu, true)
    
    if mavsdk.isReceiving() then
      tbutton_handle(VIDRES_button)
      tbutton_handle(VIDFOV_WIDE_button)
      tmenu_handle(VIDFPS_menu)
      tmenu_handle(VIDFOV_menu)
    end
    ...
    if mavsdk.isReceiving() then --and not mavsdk.gimbalIsReceiving() then
        --getMessage
        local param_value = mavlink.getMessage(mavlink.M_PARAM_VALUE) --22
        if param_value ~= nil and param_value.updated then
            --local param_id = tab2str(param_value.param_id)
            param_id = tab2str(param_value.param_id)
            if param_id == "MNT_TYPE" then
                VIDRES_value = param_value.param_value
            end    
            if param_id == "MNT_ZFLAGS" then
                VIDFOV_value = param_value.param_value
            end    
        end    
        
        if param_value ~= nil then
            lcd.drawText(LCD_W/2, 100, "RECEIVED", CUSTOM_COLOR+CENTER)
            if param_value.updated then
              lcd.drawText(LCD_W/2, 120, "updated", CUSTOM_COLOR+CENTER)
              lcd.drawText(LCD_W/2, 140, "param_id  "..tab2str(param_value.param_id), CUSTOM_COLOR+CENTER)
              local s = string.format("%s  %d", "param_value", param_value.param_value)
    ...
    tmenu_draw(VIDFPS_menu)
    tmenu_draw(VIDFOV_menu)
    
    lcd.setColor(CUSTOM_COLOR, p.WHITE)
    if VIDRES_value ~= nil then
        local s = string.format("%s  %d", "VID RES", VIDRES_value)
        lcd.drawText(LCD_W/2, 10, s, CUSTOM_COLOR+CENTER)
    else    
        lcd.drawText(LCD_W/2, 10, "VID RES nil", CUSTOM_COLOR+CENTER)
    end    
    if VIDFOV_value ~= nil then
        local s = string.format("%s  %d", "VID FOV", VIDFOV_value)
        lcd.drawText(LCD_W/2, 30, s, CUSTOM_COLOR+CENTER)
    else    
        lcd.drawText(LCD_W/2, 30, "VID FOV nil", CUSTOM_COLOR+CENTER)
    end    
