...
----------------------------------------------------------------------
local versionStr = "0.30.0 2021-07-03"
...
local function touchEvent(e)
    ...
    if touch.myEvent == e then return true end
    return false
end

local function touchEventTap(rect)
    if touch == nil then return false end
    if touch.myEvent == TEVT_TAP and
...
local function camera_button_draw()
    ...
    local radius = r.w/2
    lcd.drawCircle(xmid, r.y, radius, CUSTOM_COLOR)
    if camera_button.pressed then
        lcd.setColor(CUSTOM_COLOR, p.RED)
        lcd.drawFilledRectangle(xmid-0.6*radius, r.y-0.6*radius, 1.2*radius, 1.2*radius, CUSTOM_COLOR+SOLID)    
    else
        lcd.setColor(CUSTOM_COLOR, p.DARKRED)
        lcd.drawFilledCircle(xmid, r.y, 0.87*radius, CUSTOM_COLOR)
    end
end
