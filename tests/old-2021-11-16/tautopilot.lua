...
function apdraw:HudAt(x,y,h)
    ...
    apdraw:HudVerticalSpeedAt(x, y+h-18)
end    
...
function apdraw:GpsStatusAt(gpsId, x,y, dy)
    ...
    if gpsId > 0 then
        lcd.drawText(x, y+8, apdraw:getGpsFixStr(gpsId), CUSTOM_COLOR+txtsize1+LEFT) --MIDSIZE+LEFT)
    else
        local fixstr = "No FIX"
        if gpsfix >= mavlink.GPS_FIX_TYPE_3D_FIX then
...
function apdraw:GpsCoordsAt(sourceId, x,y)
    ...
    lcd.setColor(CUSTOM_COLOR, p.WHITE)
    lcd.drawText(x, y, apdraw:LatLonToDms(lat), CUSTOM_COLOR)
    lcd.drawText(x, y+16, apdraw:LatLonToDms(lon,lat), CUSTOM_COLOR)
end
