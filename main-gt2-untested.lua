...
-- calling gimbalSetPitchYawDeg() sets mode implicitely to MAVLink targeting
...
if debug then
  pointA.lat = 480674167; pointA.lon = 78769399
end  
...
local function doPageQuickshotRunning()
    ...
    -- set display area
    local pA_x = draw.xmid
    local pA_y = draw.ymid
...
if debug then
  cur_lat = 480674000; cur_lon = 78770000; --cur_yaw = 72
    ...
    local vy = -getValue("ele")/1024
    if vy > -0.01 and vy < 0.01 then vy = 0 end
    local speedy = 1.0 * vy
    ...
    if cablecam_state <= 1 then
        ...
        cablecam_lat0 = pointA.lat
        cablecam_lon0 = pointA.lon
    ...
    if cablecam_run and tnow - cablecam_tlast > 10 then --only every 100 ms
        ...
        cablecam_is_aty = cablecam_is_aty + speedy * 0.01*(tnow-cablecam_tlast)
        if cablecam_is_aty > 1 then cablecam_is_aty = 1 end
        if cablecam_is_aty < -1 then cablecam_is_aty = -1 end
    
        cablecam_tlast = tnow
    end    

    --local pCntrl_lat = m_to_dpos(1.5*cablecam_is_aty) + pointA.lat -- Y
    --local pCntrl_lon = m_to_dpos(1.5*cablecam_is_at) / cablecam_xScale + pointA.lon -- X
    local pCntrl_lat = pointA.lat
    local pCntrl_lon = pointA.lon
    local pCntrl_alt = pointA.alt
    local pCntrl_speed = 0.0 --math.sqrt( speed*speed + speedy*speedy )
    
    local pCntrl_yaw = pointA.yaw + 90.0 * cablecam_is_aty
    if pCntrl_yaw > 360.0 then pCntrl_yaw = pCntrl_yaw - 360.0 end
    if pCntrl_yaw < 0.0 then pCntrl_yaw = pCntrl_yaw + 360.0 end
    
    local pCntrl_pitch = 90.0 * cablecam_is_at
    if pCntrl_pitch > 0.0 then pCntrl_pitch = 0.0 end
    if pCntrl_pitch < -90 then pCntrl_pitch = -90.0 end
    ...
    local pCntrl_x = 50 * cablecam_is_at + pA_x
    local pCntrl_y = 50 * cablecam_is_aty + pA_y
    ...
    if doUpdate then
        ...
        --end
        mavsdk.apSetYawDeg(pCntrl_yaw)
        --gimbalSetPitchYawDeg(pCntrl_pitch, 0)
    end    
